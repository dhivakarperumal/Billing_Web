...
const AddProducts = () => {
    ...
    // Update Total Stock
    useEffect(() => {
        const total = formData.variants.reduce((acc, v) => acc + (Number(v.stock) || 0), 0);
            ...
            <form onSubmit={handleSubmit} className="max-w-6xl mx-auto space-y-8">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                        ...
                        <FormSection title="Core Details" icon={<FiInfo className="text-blue-500" />}>
                            <div className="space-y-6">
                                ...
                                <div className="grid md:grid-cols-3 gap-6">
                                    ...
                                    />
                                    <FormInput label="Rating (1-5)" name="rating" type="number" step="0.1" max="5" min="0" value={formData.rating} onChange={handleChange} placeholder="4.5" />
                                </div>
                            </div>
                        </FormSection>

                        <FormSection title="Pricing & Stock" icon={<FiTrendingUp className="text-emerald-500" />}>
                            <div className="grid md:grid-cols-3 gap-6">
                                <FormInput label="MRP (Base Price)" name="mrp" type="number" value={formData.pricing.mrp} onChange={(e) => handleChange(e, 'pricing')} placeholder="0.00" icon="₹" />
                                <FormInput label="Offer (%)" name="discount" type="number" value={formData.pricing.discount} onChange={(e) => handleChange(e, 'pricing')} placeholder="0" icon="%" />
                            ...
                            <div className="space-y-4">
                                {formData.variants.map((v, i) => (
                                    <div key={i} className="group relative bg-[#F8F9FF] p-5 rounded-2xl border border-indigo-50 grid md:grid-cols-7 gap-4 items-end">
                                        ...
                                        <FormInput label="Offer (%)" value={v.discount} onChange={(e) => handleVariantChange(i, "discount", e.target.value)} placeholder="0" />
                                        <FormInput label="Offer Price" value={v.sellingPrice} readOnly placeholder="0" className="bg-white/50" />
                                        <FormInput label="Stock" value={v.stock} onChange={(e) => handleVariantChange(i, "stock", e.target.value)} placeholder="0" />
                    ...
                    <div className="space-y-8">
                        ...
                        <FormSection title="Media Assets" icon={<FiImage className="text-orange-500" />}>
                            <div className="space-y-4">
                                <div className="relative h-40 border-2 border-dashed border-gray-200 rounded-2xl flex flex-col items-center justify-center bg-gray-50 hover:bg-gray-100 transition-all cursor-pointer">
                                    ...
                                    <span className="text-xs font-black text-gray-400 uppercase tracking-wider">Click to Upload Images</span>
                                    <span className="text-[10px] text-gray-300 mt-1">Up to 5 images. Max 2MB each.</span>
                                </div>
                        ...
                        <FormSection title="Expiry & Supplier" icon={<FiCalendar className="text-yellow-500" />}>
                            <div className="space-y-6">
                                <div>
                                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] block mb-2">Shelf Life Details</label>
                                    <div className="grid grid-cols-2 gap-4">
                                        <FormInput label="MFG Date" name="mfgDate" type="date" value={formData.expiry.mfgDate} onChange={(e) => handleChange(e, 'expiry')} />
                                        <FormInput label="EXP Date" name="expDate" type="date" value={formData.expiry.expDate} onChange={(e) => handleChange(e, 'expiry')} />
                                    </div>
                                    <div className="mt-4">
                                        <FormInput label="Batch Number" name="batchNo" value={formData.expiry.batchNo} onChange={(e) => handleChange(e, 'expiry')} placeholder="B-882-X" />
                                    </div>
                                </div>

                                <div className="pt-6 border-t border-gray-100">
                                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] block mb-2">Supplier Information</label>
                                    <FormInput label="Supplier Name" name="name" value={formData.supplier.name} onChange={(e) => handleChange(e, 'supplier')} placeholder="WholeSale Corp" />
                                    <div className="mt-4">
                                        <FormInput label="Contact Info" name="contact" value={formData.supplier.contact} onChange={(e) => handleChange(e, 'supplier')} placeholder="+91 98765 43210" />
                                    </div>
                                </div>
                            </div>
                        </FormSection>
...
const FormInput = ({ label, icon, ...props }) => (
    <div className="flex flex-col gap-2.5">
        ...
        <div className="relative">
            {icon && (
                <span className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-400 font-bold text-xs">{icon}</span>
            )}
            <input
                {...props}
